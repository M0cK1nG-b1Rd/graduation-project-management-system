...
import com.gms.common.utils.HttpContextUtil;
import com.gms.common.utils.IPUtil;
...
//        return simpleAuthorizationInfo;
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        String encryptToken = GmsUtil.encryptToken(token);
        String encryptTokenInRedis = null;
        try {
            encryptTokenInRedis = redisService.get(GmsConstant.TOKEN_CACHE_PREFIX + encryptToken + "." + ip);
        ...
        if (!JWTUtil.verify(token, username, user.getPassword()))
            throw new AuthenticationException("token校验不通过");
        return new SimpleAuthenticationInfo(token, token, "gms_shiro_realm");
    }
}
